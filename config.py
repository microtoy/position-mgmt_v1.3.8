...
data_source_dict = {
    ...
    "coin-btc": ('load_coin_btc', '/Users/microtoy/Documents/QuantWin/data/coin-binance-candle-csv-1h'),
}
...
# ==== factor_col_limit 介绍 ====
factor_col_limit = 128  # [优化] 针对 M4 24GB 内存，提升单次计算因子列数 (基准: 64 -> 128)
# - 数字越大，计算速度越快，但同时内存占用也会增加。
# - 该数字是在 "因子数量 * 参数数量" 的基础上进行优化的。
#   - 例如，当你遍历 200 个因子，每个因子有 10 个参数，总共生成 2000 列因子。
#   - 如果 `factor_col_limit` 设置为 128，则计算会拆分为 ceil(2000 / 128) = 16 个批次。
# - 对于16GB内存的电脑，在跑含现货的策略时，64是一个合适的设置。
# - 针对 24GB+ 内存，可以安全提升至 128 或更高。
